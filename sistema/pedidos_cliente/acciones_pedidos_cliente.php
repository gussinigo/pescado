	...
	switch ($mode){
		...
		case "insert":
				...
				if(!$insert){
					...
					for($i=0; $i < count($listadesc); $i++){
						...
						/*$qr = "insert into pedidos_cliente_detalle (idsolicitud,idproducto,cantidad,idmedida,peso,peso_idmedida,idestatus,precio_unitario,importe) values ('".$lastID."','".$idproducto."','".$cantidad."','".$idumedida."','".$peso."','".$idumedidapeso."','1','".$row_qr1['costo_venta']."','".$costo.") ";*/
				...
				if(!$update){
					...
					if(!$delete){
						...
						for($i=0; $i < count($listadesc); $i++){
							...
							$busqueda = "select count(idsolicituddet) as total from pedidos_cliente_detalle where idsolicitud = '".$hdnid."' and idproducto = '".$idproducto."'";
							$res_busqueda = mysqli_query($link, $busqueda);
							$row_busqueda = mysqli_fetch_array($res_busqueda);
							
							
							if($row_busqueda['total'] == '0'){
							
								$qr = "insert into pedidos_cliente_detalle (idsolicitud,idproducto,cantidad,idmedida,peso,peso_idmedida,idestatus,precio_unitario,importe) values ('".$hdnid."','".$idproducto."','".$cantidad."','".$idumedida."','".$peso."','".$idumedidapeso."','1','".$row_qr1['costo_venta']."','".$costo."') ";
								$insert2 = mysqli_query($link, $qr);
		...
		case "peso":
				...
				if(!$update){
					...
					//$sql_del = "delete from pedidos_cliente_detalle where idsolicitud = ".$hdnid;
					$sql_del = "delete from pedidos_cliente_detalle where idestatus = 1 and idsolicitud = ".$hdnid;
					$delete = mysqli_query($link, $sql_del);
